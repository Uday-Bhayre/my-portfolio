...
import {
  Github, Linkedin, Mail, Phone, Download, ArrowRight, ExternalLink,
  Code2, Server, Cloud, Smartphone, Database, Wrench, Trophy, Briefcase,
  GraduationCap, Sparkles, MapPin, ChevronDown, Star, GitFork,
  CheckCircle, Loader2,
} from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Backend Software Engineer & IIITM Gwalior CS graduate. Building scalable distributed systems, fintech infra, and high-performance backends." },
      { property: "og:title", content: "Uday Bhayre — Backend Software Engineer" },
      { property: "og:description", content: "Backend engineer. Mysa intern, LeetCode Knight, CodeChef 4-star. 1000+ DSA problems solved." },
    ],
  }),
  component: Portfolio,
});
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "py-3" : "py-5"}`}>
      <div className={`mx-auto max-w-7xl px-4 sm:px-6`}>
        <div className={`flex items-center justify-between rounded-full px-5 py-3 transition-all ${scrolled ? "glass shadow-[var(--shadow-card)]" : ""}`}>
          <a href="#home" className="flex items-center gap-2.5">
            <div className="relative h-9 w-9 rounded-full" style={{ background: "var(--gradient-primary)" }}>
              <span className="absolute inset-0 flex items-center justify-center font-display font-bold text-primary-foreground">U</span>
            </div>
            <span className="font-display text-lg font-bold tracking-tight">Uday<span className="text-primary">.</span></span>
          </a>
          <nav className="hidden md:flex items-center gap-1">
            {NAV.map(n => (
              <a key={n.id} href={`#${n.id}`} className="rounded-full px-4 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-white/5 transition">
                {n.label}
              </a>
            ))}
          </nav>
          <a href="#contact" className="hidden md:inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium text-primary-foreground transition hover:opacity-90"
             style={{ background: "var(--gradient-primary)", boxShadow: "var(--shadow-glow)" }}>
            Let's Talk
          </a>
          <button onClick={() => setOpen(!open)} className="md:hidden rounded-full glass p-2.5" aria-label="Menu">
            <div className="space-y-1.5">
              <div className={`h-0.5 w-5 bg-foreground transition ${open ? "translate-y-2 rotate-45" : ""}`} />
        ...
        {open && (
          <div className="md:hidden mt-2 glass rounded-2xl p-4 flex flex-col gap-1">
            {NAV.map(n => (
              <a key={n.id} href={`#${n.id}`} onClick={() => setOpen(false)} className="rounded-lg px-4 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-white/5">
                {n.label}
              </a>
            ))}
          </div>
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (!del) {
        ...
        const next = w.slice(0, text.length - 1);
        setText(next);
        if (next === "") { setDel(false); setI((i + 1) % words.length); }
      }
    }, del ? speed / 2 : speed);
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-28 pb-16">
      ...
      <div className="absolute inset-0 -z-10 opacity-[0.07]"
           style={{ backgroundImage: "linear-gradient(var(--color-foreground) 1px, transparent 1px), linear-gradient(90deg, var(--color-foreground) 1px, transparent 1px)", backgroundSize: "60px 60px" }} />
      <div className="mx-auto max-w-7xl px-4 sm:px-6 grid lg:grid-cols-2 gap-12 items-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <p className="font-display text-xl mb-3" style={{ color: "var(--amber)" }}>Hi there, I am</p>
          ...
          <p className="text-base text-muted-foreground max-w-xl leading-relaxed mb-8">
            CS graduate from <span className="text-foreground font-medium">IIITM Gwalior</span> building
            scalable backend systems, real-time applications. Experienced in fintech payment systems, event-driven architectures, and production-grade software development.
          </p>

          <div className="flex flex-wrap gap-3 mb-10">
            <a href="#projects" className="group inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-medium text-primary-foreground transition hover:scale-[1.02]"
               style={{ background: "var(--gradient-primary)", boxShadow: "var(--shadow-glow)" }}>
              View Projects <ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" />
            </a>
            <a href="#contact" className="inline-flex items-center gap-2 rounded-full border border-primary/40 px-6 py-3 text-sm font-medium text-primary hover:bg-primary/10 transition">
          ...
          <div className="flex items-center gap-5">
            {[
              ...
              { icon: Linkedin, href: "https://www.linkedin.com/in/uday-bhayre/", label: "LinkedIn" },
            ...
            <span className="text-xs text-muted-foreground inline-flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" />India</span>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.9, delay: 0.15 }}
          ...
          <div className="orb animate-float-slow absolute top-4 left-8 h-44 w-44 sm:h-56 sm:w-56" style={{ background: "var(--gradient-orb-coral)" }} />
          <div className="orb animate-float-medium absolute top-20 right-2 h-16 w-16 sm:h-20 sm:w-20" style={{ background: "var(--gradient-orb-mint)" }} />
          <div className="orb animate-float-slow absolute bottom-12 right-10 h-24 w-24 sm:h-32 sm:w-32" style={{ background: "var(--gradient-orb-amber)" }} />
          <div className="relative z-10 h-[380px] w-[300px] sm:h-[440px] sm:w-[340px] rounded-2xl overflow-hidden glass"
               style={{ boxShadow: "var(--shadow-glow)" }}>
            <img src="https://i.postimg.cc/RFkQ7dhc/Uday.jpg" alt="Uday Bhayre" width={1024} height={1024} className="h-full w-full object-cover brightness-[0.85]" />
      ...
      <a href="#about" className="absolute bottom-6 left-1/2 -translate-x-1/2 text-muted-foreground hover:text-foreground transition" aria-label="Scroll down">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 text-center">
        <Reveal>
          <p className="font-display text-2xl sm:text-4xl lg:text-5xl font-semibold leading-tight">
            ...
            <span className="text-gradient">clean and scalable</span> solutions.
            I have a genuine passion for crafting backends that perform at scale.
            <span className="text-primary">"</span>
          </p>
        </Reveal>
      </div>
...
function Reveal({ children, delay = 0, className }: { children: React.ReactNode; delay?: number; className?: string }) {
  ...
  return (
    <motion.div ref={ref} className={className} initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay }}>
      {children}
    </motion.div>
  );
}
          ...
          <Reveal className="lg:col-span-2">
            <article className="glass rounded-3xl p-8 sm:p-10 h-full">
              ...
              <h3 className="font-display text-2xl sm:text-3xl md:text-4xl font-semibold leading-tight mb-5">
                I build backends that <span className="text-gradient">scale quietly</span> — and break loudly only in tests.
              </h3>
              <p className="text-muted-foreground leading-relaxed mb-4">
                CS graduate from IIITM Gwalior building scalable backend systems, real-time applications. Experienced in fintech payment systems, event-driven architectures, and production-grade software development.
              </p>
            </article>
          </Reveal>
            ...
            <div className="flex flex-col gap-6 h-full">
              <div className="glass rounded-3xl p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="rounded-xl p-2.5" style={{ background: "var(--gradient-primary)" }}>
                    <GraduationCap className="h-5 w-5 text-primary-foreground" />
                  </div>
                  <span className="text-xs uppercase tracking-wider text-muted-foreground">Education</span>
                </div>
                <p className="font-display font-semibold text-lg leading-tight">B.Tech, Computer Science</p>
                <p className="text-sm text-muted-foreground mt-1">IIITM Gwalior · Class of 2026</p>
                <div className="mt-5 flex items-end gap-2">
                  <span className="text-gradient font-display text-4xl font-bold leading-none">8.58</span>
              ...
              <div className="glass rounded-3xl p-6 flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                  Open to exciting Software Engineering opportunities and available for immediate joining.
                </p>
              </div>
            </div>
          </Reveal>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
              {focus.map(f => (
                <div key={f.k} className="border-l border-primary/40 pl-4">
                  <div className="font-display font-semibold text-sm text-primary mb-2 uppercase tracking-wider">{f.k}</div>
                ...
                <div className="relative grid lg:grid-cols-12">
                  ...
                  <div className="lg:col-span-4 p-8 sm:p-10 border-b lg:border-b-0 lg:border-r border-border relative">
                    <div className="absolute top-8 right-8 lg:hidden">
                      <span className="rounded-full bg-primary/10 text-primary text-xs px-3 py-1 font-medium">{e.tag}</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground mb-6">
                      <span className="h-1.5 w-1.5 rounded-full bg-primary" />
                      <span>0{i + 1} / 0{EXP.length}</span>
                    </div>
                    <div className="font-display text-5xl sm:text-6xl font-bold leading-none mb-4 text-gradient">
                      {e.company}
                    </div>
                  ...
                  <div className="lg:col-span-8 p-8 sm:p-10">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-4 border-t border-border">
                      {e.tech.map(t => (
                        <span key={t} className="rounded-md bg-white/5 border border-border px-2.5 py-1 text-[11px] font-mono text-muted-foreground hover:text-primary hover:border-primary/30 transition">
                          {t}
                        </span>
                      ))}
                    </div>
...
/* ---------------- SKILLS ---------------- */
const SKILLS = [
  { icon: Code2, title: "Languages", items: ["C++", "Java", "Python", "JavaScript", "Dart", "SQL"] },
  { icon: Server, title: "Backend", items: ["Spring Boot", "Node.js", "Express.js", "REST APIs", "WebSockets", "Microservices"] },
  { icon: Cloud, title: "Cloud & Data", items: ["AWS", "S3", "Step Functions", "EventBridge", "SQS", "MySQL", "MongoDB", "Redis", "Firestore"] },
  { icon: Smartphone, title: "Mobile", items: ["Flutter", "Dart", "Getx", "BLoC", "Provider"] },
  { icon: Database, title: "Architecture", items: ["Distributed Systems", "System Design", "Event-Driven", "Idempotency", "Caching"] },
  { icon: Wrench, title: "Tools", items: ["Docker", "Git", "GitHub", "CI/CD", "Firebase", "Postman", "Apidog", "DataDog", "Coralogix", "Confluence", "Agile"] },
];
...
function Skills() {
  return (
    <section id="skills" className="py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionHeader kicker="Skills" title="My toolkit" desc="Technologies I use to design, build, and ship reliable systems." />
          ...
          {SKILLS.map((s, i) => (
            <Reveal key={s.title} delay={i * 0.05}>
              <div className="group glass rounded-3xl p-6 h-full hover:border-primary/40 transition relative overflow-hidden">
                <div className="absolute -top-12 -right-12 h-32 w-32 rounded-full opacity-0 group-hover:opacity-20 transition" style={{ background: "var(--gradient-primary)", filter: "blur(40px)" }} />
                <div className="relative">
                  <div className="inline-flex rounded-2xl p-3 mb-4" style={{ background: "var(--gradient-primary)" }}>
                    <s.icon className="h-5 w-5 text-primary-foreground" />
                  </div>
                  <h3 className="font-display text-lg font-semibold mb-3">{s.title}</h3>
                  <div className="flex flex-wrap gap-2">
                    {s.items.map(it => (
                      <span key={it} className="rounded-lg bg-white/5 border border-border px-2.5 py-1 text-xs text-muted-foreground group-hover:text-foreground transition">{it}</span>
                    ))}
                  </div>
                </div>
              </div>
...
/* ---------------- SERVICES ---------------- */
const SERVICES = [
  { icon: Server, title: "Backend Development", desc: "Production-grade REST APIs, microservices, and event-driven systems built for scale.", stack: "Node.js · Spring Boot · AWS" },
  { icon: Code2, title: "Full Stack Development", desc: "End-to-end product engineering from database schema to polished UI.", stack: "React · Node.js · MongoDB" },
  { icon: Smartphone, title: "Mobile App Development", desc: "Cross-platform mobile apps with real-time features and clean UX.", stack: "Flutter · Firebase · WebSockets" },
];
...
function Projects() {
  return (
    <section id="projects" className="py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionHeader kicker="Projects" title="Selected work" desc="A few projects I'm proud of. More on GitHub." />
          ...
          {PROJECTS.map((p, i) => (
            <Reveal key={p.name} delay={i * 0.08}>
              <div className="glass rounded-3xl p-7 h-full group hover:border-primary/40 transition relative overflow-hidden">
                <div className="absolute -top-20 -right-20 h-56 w-56 rounded-full opacity-30 group-hover:opacity-50 transition" style={{ background: p.accent, filter: "blur(40px)" }} />
                <div className="relative">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {p.tech.map(t => (
                      <span key={t} className="rounded-md bg-white/5 border border-border px-2 py-0.5 text-[11px] text-muted-foreground">{t}</span>
                    ))}
                  </div>
                  <div className="flex gap-3">
                    <a href="https://github.com/Uday-Bhayre" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm text-primary hover:text-primary-glow">
                      <Github className="h-4 w-4" /> Source
                    </a>
                    {p.demo && (
                      <a href="https://github.com/Uday-Bhayre" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        ...
        <Reveal delay={0.2}>
          <div className="mt-10 text-center">
            <a href="https://github.com/Uday-Bhayre" target="_blank" rel="noreferrer"
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <Reveal>
          <div className="glass rounded-3xl p-6 flex flex-wrap items-center justify-between gap-4">
            <p className="text-sm text-muted-foreground">Find me on competitive programming platforms:</p>
            <div className="flex flex-wrap gap-2">
              {PROFILES.map(p => (
  ...
  return (
    <section id="contact" className="py-24 sm:py-32">
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <SectionHeader kicker="Contact" title="Let's build something" desc="Open to backend and app development roles, freelance projects, and collaborations." />
        <div className="grid lg:grid-cols-2 gap-6">
          <Reveal>
            <div className="glass rounded-3xl p-7 space-y-4">
              {[
                { icon: Mail, label: "Email", value: "udaybhayre@gmail.com", href: "mailto:udaybhayre@gmail.com" },
                { icon: Phone, label: "Phone", value: "+91 9329958590", href: "tel:+919329958590" },
                { icon: Linkedin, label: "LinkedIn", value: "uday-bhayre", href: "https://www.linkedin.com/in/uday-bhayre/" },
                { icon: Github, label: "GitHub", value: "Uday-Bhayre", href: "https://github.com/Uday-Bhayre" },
              ].map(c => (
                <div key={c.label} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-white/5 transition group">
                  <div className="rounded-xl bg-primary/10 p-3"><c.icon className="h-5 w-5 text-primary" /></div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-muted-foreground">{c.label}</div>
                    <a href={c.href} target="_blank" rel="noreferrer" className="font-medium truncate block hover:text-primary">{c.value}</a>
                  </div>
                  <button onClick={() => copy(c.value, c.label)} className="text-xs text-muted-foreground hover:text-primary opacity-0 group-hover:opacity-100 transition">
          ...
          <Reveal delay={0.1}>
            <form onSubmit={handleSubmit} className="glass rounded-3xl p-7 space-y-4">
              ...
              {status === "success" && (
                <p className="text-xs text-center" style={{ color: "var(--mint)" }}>Message sent successfully. I'll get back to you soon!</p>
              )}
              {status === "error" && (
                <p className="text-xs text-center text-red-400">Something went wrong. Please try again or reach out directly via email.</p>
              )}
            </form>
          </Reveal>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 flex flex-col items-center gap-6">
        <div className="flex items-center gap-5">
          {[
            ...
            <a key={i} href={s.href} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-primary transition">
              <s.icon className="h-5 w-5" />
            </a>
          ))}
        </div>
        <div className="text-center text-xs text-muted-foreground">
          Built with <span className="text-primary">♥</span> by Uday Bhayre · © {new Date().getFullYear()}
        </div>
      </div>
    </footer>
  );
